...
3. **`moteid` is globally unique, not session-scoped — and is `session.number`.** Stortinget assigns each meeting a 5-digit ID (`11518`) unique across all sesjoner and stortingsperioder: the natural primary key, also what the video archive keys on, and the value the parser emits as `meta.session` (`22_11518`), the `originID` prefix, and `session.number`. It guarantees uniqueness but **overflows the platform's 4-digit `session.number` slot** (§3.1). The slot-fitting alternative — a computed sesjon-year ordinal, with `moteid` kept only as `originID` — belongs to §3.4's electoralPeriod redesign (sesjon as `subPeriod`, scoping `session.number` to it).
...
| NO | 3 (022) | 5 (11517) | `NO-02211517` (8) | **mis-parsed as media** (length ≥ 8): `session.number = moteid` (globally-unique 5-digit) overflows the 4-digit slot — same class as EU. The slot-fitting alternative (sesjon-year ordinal) is the §3.4 fix |
...
**Fix:**
- **Tools, recommended:** Stop encoding semantic information into oversized integer fields. Allocate sequential `session.number` for EU (plenary-day counter within term). For SE, switch `electoralPeriod.number` semantics to *valperiod* (4-year electoral term) and use riksmöte as a `subPeriod` (see §4.2). For NO, scope `session.number` to a computed sesjon-year ordinal via `subPeriod` and keep the *globally-unique `moteid`* as `originID` only (a small slot-fitting integer); NO currently sets `session.number = moteid`, which is unique but slot-overflowing. All the recommended values are small integers that fit the slots.
...
- **Only the media stream has source-side per-speech identity** (DE-SH, DE-BY, DE-BW, DE-HH, DE-NI, DE-NW, DE-SN: video ✓, the text is not per-speech at source). The media spine is authoritative; per-speech text is **reconstructed and joined onto it** rather than read off the source — for the PDF Landtage via `optv.shared.pdf2tei` (PDF → TEI → text grafted on by speaker/agenda match), and for DE-NI via time-aligned WebVTT subtitles attached by `speechIndex`. Matched speeches carry `textContents`; unmatched ones keep `[]`, and the per-speech clip + speaker + agenda metadata is a valuable artefact on its own. **These text paths are experimental and unvalidated (no Whisper-QC/text-fidelity audit) — not yet platform-ready;** the reconstruction quality (PDF extraction + text↔spine join, or VTT calibration) is the open problem, parliament by parliament.
